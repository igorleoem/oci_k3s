...
Terraform state is stored in OCI Object Storage via the S3-compatible API. Before initialising, generate Customer Secret Keys:

1. OCI Console → Profile → User Settings → Customer Secret Keys → Generate Secret Key
2. Export the keys:
...
│   ├── *.tf                   # Root-level Terraform files (variables, outputs, backend, providers)
│   ├── modules/               # Child modules
│   │   ├── networking/        # VCN, subnets, security lists
│   │   ├── compute/           # Ampere A1 instances, cloud-init
│   │   ├── storage/           # OCI Object Storage bucket (Terraform state)
│   │   ├── secrets/           # OCI Vault secrets
│   │   ├── iam/               # IAM policies and dynamic groups
│   │   ├── loadbalancer/      # Network Load Balancer
│   │   └── manifests/         # ArgoCD manifest generation
